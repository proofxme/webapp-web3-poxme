...
export default function NewIdentity() {
  ...
  const validateHandler = (email: string) => {
    ...
    const validEmailName = /^[a-zA-Z0-9._%+-]+$/;
    return validEmailName.test(email);
  };
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      await createIdentity({displayName, handler, bio, visibility, active, privacy});
    } catch (error) {
      console.log(error)
        ...
        <div className="grid gap-4">
          <Card className="w-full max-w-screen-lg">
            ...
            <CardFooter>
              ...
              <Button className="ml-auto">Create Identity</Button>
            </CardFooter>
          </Card>
        </div>
      </div>
